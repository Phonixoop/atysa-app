...
class functions
{
    ...
    public function iterable_to_traversable(iterable $it): Traversable
    {
        yield from $it;
    }

    // our code
    public function gotoZarinpal($fee, $plateCount, $mobile)
    {

        $data = array(
            "merchant_id" => "5f5367d4-333f-11e8-b507-005056a205be",
            "amount" => $fee,
            "callback_url" => "http://localhost:8000/pay/zarinpalverify.php",
            "description" => "خرید برای " . $plateCount . " تعداد غذا",
            "metadata" => ["email" => "sale@atysa.ir", "mobile" => $mobile],
        );
        ...
        if ($err) {
            ...
            if (empty($result['errors'])) {
                if ($result['data']['code'] == 100) {
                    return "https://api.zarinpal.com/pg/StartPay/" . $result['data']["authority"];
