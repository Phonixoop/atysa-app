    ...
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            ...
            >افزودن برنامه غذایی جدید برای پلن 
                <a href="/admin/plans/single/{{$plan->id}}" style="color: #1D9FA4;;">{{$plan->name}}</a>
            </h4>
            @if(isset($plan["months"]))
            <div style="display: flex">
                <div style="display: flex; gap:5px;">
                    @foreach($plan["months"] as $item)
                    <a 
                    href="/admin/plans/months/single/{{$plan->id}}/{{$item["month"]}}"
                    style="background-color:#1D9FA4; color:#ffffff; text-align: center; border-radius: 1000px; min-width: 50px;">{{$item["month"]}}</a>
                    @endforeach
                </div>
            </div>
            @endif
